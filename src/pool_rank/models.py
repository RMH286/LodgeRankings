...
# Default values for initializing a new score
MU = 25.0
SIGMA = 25.0 / 3.0


class PoolScore(models.Model):
    """Class used to model a players score for pool.
    
        FIELDS:
            players         [One to One Field]
                            reference to a Player who this PoolScore describes
                            (see models.Player in house app)
            games_played    [Integer Field]
                            total number of pool games played
            wins            [Integer Field]
                            total number of pool wins
